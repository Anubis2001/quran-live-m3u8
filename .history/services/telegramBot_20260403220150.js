  ...
  start() {
    ...
    try {
      ...
      const launchTimeout = setTimeout(() => {
        if (!this.isRunning) {
          console.warn('⚠️  Telegram bot connection is taking longer than expected...');
          console.warn('   The bot will continue trying to connect in the background.');
          console.warn('   If it fails, check your network connection and bot token.');
        }
      }, 15000);
      ...
      this.bot.launch({ dropPendingUpdates: true }).then(() => {
        ...
        console.log(`   Bot Username: @${this.bot.botInfo?.username || 'N/A'}`);
        console.log(`   Bot ID: ${this.bot.botInfo?.id || 'N/A'}`);
        console.log(`   Authorized Admins: ${this.adminIds.length} user(s)`);
        this.adminIds.forEach(id => console.log(`     - User ID: ${id}`));
        console.log('━'.repeat(60));
        console.log('💡 Tip: Send /start to your bot on Telegram to begin!\n');
      }).catch(err => {
        clearTimeout(launchTimeout);
        console.error('\n❌ Failed to start Telegram bot:', err.message);
        console.error('   Error Code:', err.code || 'N/A');
        console.error('   Description:', err.description || 'N/A');
        console.error('\n   Common solutions:');
        console.error('   1. Verify your bot token is correct');
        console.error('   2. Check your internet connection');
        console.error('   3. Ensure Telegram API is not blocked by firewall');
        console.error('   4. Try regenerating the token via @BotFather\n');
        this.isRunning = false;
      });
      
      // Immediate feedback that launch has been initiated
      console.log('✓ Launch initiated - waiting for connection confirmation...\n');
