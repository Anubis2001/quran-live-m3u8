...
function startStream(name, filePath) {
  ...
  // ALTERNATIVE: Try direct shell command execution if spawn fails
  const shellCommand = `cd "${dir}" && ffmpeg -re -stream_loop -1 -i "${filePath}" -c:a copy -hls_time 3 -hls_list_size 6 -hls_flags delete_segments+round_durations -hls_segment_filename "${outputSegment}" -hls_segment_type mpegts "${outputPlaylist}"`;
  console.log(`\nAlternative shell command (if spawn fails):`);
