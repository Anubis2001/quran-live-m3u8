...
async function startYoutubeStream(name, mediaUrl, cookiesPath) {
  ...
  return new Promise((resolve, reject) => {
    exec(downloadCommand, { cwd: streamDir, timeout: 300000 }, async (error, stdout, stderr) => {
      ...
      // NOTE: Universal streams are NOT saved to streams.json
      // They are temporary session-based streams that won't persist across restarts
      // This is intentional - only uploaded streams should be persistent
