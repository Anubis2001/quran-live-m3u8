  ...
  waitForStreamName(ctx, audioFilePath, expectedUserId) {
    const handler = async (ctx) => {
      ...
      try {
        ...
        setTimeout(() => {
          // Get the base URL - use env variable or auto-detect server IP
          let baseUrl = process.env.TELEGRAM_STREAM_BASE_URL;
          
          // If not set or still localhost, try to get the actual server IP
          if (!baseUrl || baseUrl.includes('localhost')) {
            ...
            baseUrl = `http://${serverIP}:${process.env.PORT || 8300}`;
          }
