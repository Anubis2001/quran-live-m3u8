...
function restoreStreams() {
  ...
  streamsMetadata = streamsMetadata.filter(streamData => {
    ...
    if (streamData.filePath === '[protected]' || !streamData.filePath) {
      console.warn(`⚠️  Removing invalid stream entry (no file path): ${streamData.name}`);
      return false;
    }
    
    // Check if the actual file exists
    if (!fs.existsSync(streamData.filePath)) {
      console.warn(`⚠️  Removing stream with missing file: ${streamData.name} (${streamData.filePath})`);
      return false;
    }
