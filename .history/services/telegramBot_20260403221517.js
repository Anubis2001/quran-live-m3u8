  ...
  start() {
    ...
    try {
      ...
      this.bot.telegram.getMe().then((botInfo) => {
        ...
        console.error('\n❌ Failed to connect to Telegram:', err.message);
        console.error('   Please check your token and network connection.');
        this.isRunning = false;
      });
  ...
  waitForStreamName(ctx, audioFilePath, expectedUserId) {
    const handler = async (ctx) => {
      ...
      try {
        ...
        setTimeout(() => {
          // Get the display URL from environment variable (what users see in messages)
          const displayUrl = process.env.TELEGRAM_STREAM_DISPLAY_URL || `http://localhost:${process.env.PORT || 8300}`;
          const streamUrl = `${displayUrl}/streams/${streamName}/stream.m3u8`;
          ...
          const successMessage = 
            ...
            `💡 *How to use:*\n` +
            `• Copy the URL above and paste it in any HLS player\n` +
            `• Or use it in your web app with an audio/video tag\n` +
            `• The stream will loop infinitely\n\n` +
            `📊 *Stream Details:*\n` +
            `• Format: HLS (HTTP Live Streaming)\n` +
            `• Codec: Audio only (AAC/MP3)\n` +
            `• Status: 🟢 Live`;
          
          ctx.reply(successMessage, { parse_mode: 'Markdown' });
          console.log(`[Telegram] Stream URL sent: ${streamUrl}`);
        }, 2000);
