...
function listStreams(req) {
  ...
  // Get the correct base URL using proxy-aware detection
  // Priority: X-Forwarded-Host > req.get("host") > localhost fallback
  const forwardedHost = req.get('X-Forwarded-Host');
  const actualHost = forwardedHost || req.get('host') || 'localhost';
  ...
  return streamsMetadata.map(streamData => {
    ...
    return { 
      ...
      url: `${baseUrl}/streams/${streamData.name}/stream.m3u8`,
...
/**
 * Rename a stream (supports Arabic and Unicode characters)
 * @param {string} oldName - Current stream name
 * @param {string} newName - New stream name (can include Arabic/Unicode)
 * @returns {Promise<Object>} Result with success status and new URL
 */
async function renameStream(oldName, newName) {
  console.log(`\n🔄 Renaming stream: "${oldName}" → "${newName}"`);
  
  // Validate new name (allow Arabic, Unicode, letters, numbers, spaces, hyphens, underscores)
  if (!/^\p{L}[\p{L}\p{N}\s_-]*$/u.test(newName)) {
    return { 
      success: false, 
      error: 'Invalid stream name. Must start with a letter and can contain letters, numbers, spaces, hyphens, and underscores.'
    };
  }
  
  // Check if stream exists in metadata
  const streamsMetadata = loadStreamsMetadata();
  const streamIndex = streamsMetadata.findIndex(s => s.name === oldName);
  
  if (streamIndex === -1) {
    return { success: false, error: `Stream "${oldName}" not found` };
  }
  
  // Check if new name already exists
  if (streamsMetadata.some(s => s.name === newName)) {
    return { success: false, error: `A stream named "${newName}" already exists` };
  }
  
  const streamData = streamsMetadata[streamIndex];
  const oldPath = path.join(__dirname, '..', 'streams', oldName);
  const newPath = path.join(__dirname, '..', 'streams', newName);
  
  // Check if old directory exists
  if (!fs.existsSync(oldPath)) {
    return { success: false, error: `Stream directory not found for "${oldName}"` };
  }
  
  try {
    // Stop the stream if it's running
    const isRunning = runningStreams[oldName] !== undefined;
    if (isRunning) {
      console.log(`⏸️  Stopping stream "${oldName}" before rename...`);
      await stopStream(oldName);
    ...
    console.log(`✓ Directory renamed: ${oldName} → ${newName}`);
    
    // Update metadata
    streamData.name = newName;
    
    // Update file path to use new directory
    const fileName = path.basename(streamData.filePath);
    streamData.filePath = path.join(newPath, fileName);
    
    streamsMetadata[streamIndex] = streamData;
    saveStreamsMetadata(streamsMetadata);
    console.log(`✓ Metadata updated`);
    
    // Restart the stream if it was running
    if (isRunning) {
      console.log(`▶️  Restarting stream with new name...`);
      startStream(newName, streamData.filePath);
    }
    
    // Construct the new URL
    const baseUrl = `${process.env.TELEGRAM_STREAM_DISPLAY_URL || 'http://localhost:' + (process.env.PORT || 8300)}`;
    const newUrl = `${baseUrl}/streams/${encodeURIComponent(newName)}/stream.m3u8`;
    ...
    console.error(`❌ Error renaming stream:`, err.message);
    return { success: false, error: err.message };
  }
}

/**
 * Comprehensive stream synchronization between filesystem and streams.json
 * Performs bidirectional sync during server startup
 */
function restoreStreams() {
  console.log('\n' + '='.repeat(60));
  console.log('🔄 STREAM SYNCHRONIZATION STARTED');
  console.log('='.repeat(60));
  
  let streamsMetadata = loadStreamsMetadata();
  const base = path.join(__dirname, "..", "streams");
  
  // ===== PHASE 1: VALIDATE EXISTING METADATA =====
  console.log('\n📋 Phase 1: Validating metadata entries...');
  const validMetadata = [];
  let removedCount = 0;
  
  streamsMetadata.forEach(streamData => {
    // Remove entries with invalid file paths
    if (streamData.filePath === '[protected]' || !streamData.filePath) {
      console.warn(`   ⚠️  Removing invalid entry (no file path): ${streamData.name}`);
      removedCount++;
      return;
    }
    ...
    if (!fs.existsSync(streamData.filePath)) {
      console.warn(`   ⚠️  Removing entry (file missing): ${streamData.name}`);
      removedCount++;
      return;
    }
    
    // Keep valid entries
    validMetadata.push(streamData);
  });
  
  streamsMetadata = validMetadata;
  console.log(`   ✓ Validated: ${streamsMetadata.length} entries remain`);
  if (removedCount > 0) {
    console.log(`   🗑️  Removed: ${removedCount} invalid entries`);
  }
  
  // ===== PHASE 2: SCAN FILESYSTEM =====
  console.log('\n📁 Phase 2: Scanning streams directory...');
  ...
  if (fs.existsSync(base)) {
    ...
    folders.forEach(name => {
      ...
      // Prioritize source media files over playlists
      const mediaFile = audioFiles[0] || videoFiles[0];
      
      if (mediaFile) {
        const filePath = path.join(folder, mediaFile);
        discoveredStreams.set(name, {
          name: name,
          filePath: filePath,
          mediaType: audioFiles[0] ? 'audio' : 'video',
          fileName: mediaFile
        });
        console.log(`   ✓ Found: ${name} (${mediaFile})`);
      } else if (playlistFiles.length > 0) {
        // Directory has HLS playlists but no source media
        console.log(`   ℹ️  HLS only: ${name} (${playlistFiles.length} playlists)`);
      }
    });
    
    console.log(`   ✓ Scanned: ${scannedDirs} directories`);
    console.log(`   ✓ Discovered: ${discoveredStreams.size} streams with media`);
  } else {
    console.log('   ⚠️  Streams directory does not exist');
  }
  
  // ===== PHASE 3: ADD MISSING STREAMS =====
  console.log('\n➕ Phase 3: Adding missing streams to registry...');
  let addedCount = 0;
  
  discoveredStreams.forEach((streamInfo, name) => {
  ...
  if (addedCount > 0) {
    console.log(`   ✓ Added: ${addedCount} new stream(s) to registry`);
  } else {
    console.log('   ✓ No new streams to add');
  }
  
  // ===== PHASE 4: REMOVE ORPHANED ENTRIES =====
  console.log('\n➖ Phase 4: Removing orphaned metadata entries...');
  const initialCount = streamsMetadata.length;
  
  streamsMetadata = streamsMetadata.filter(streamData => {
    // Check if directory still exists
    const streamDir = path.dirname(streamData.filePath);
    const dirExists = fs.existsSync(streamDir);
    
    if (!dirExists) {
      console.warn(`   🗑️  Removing orphaned entry: ${streamData.name}`);
      return false;
    }
    
    // Check if media file still exists
    const fileExists = fs.existsSync(streamData.filePath);
    if (!fileExists) {
      console.warn(`   🗑️  Removing entry (file deleted): ${streamData.name}`);
      return false;
    }
  ...
  const orphanedCount = initialCount - streamsMetadata.length;
  if (orphanedCount > 0) {
  ...
  // ===== PHASE 5: SAVE SYNCHRONIZED METADATA =====
  console.log('\n💾 Phase 5: Saving synchronized metadata...');
  saveStreamsMetadata(streamsMetadata);
  console.log(`   ✓ Metadata saved (${streamsMetadata.length} streams)`);
  
  // ===== PHASE 6: START STREAMS =====
  console.log('\n🚀 Phase 6: Starting streams...');
  let startedCount = 0;
  let failedCount = 0;
  ...
  streamsMetadata.forEach(streamData => {
    if (fs.existsSync(streamData.filePath)) {
      try {
        ...
        console.log(`   ▶️  Started: ${streamData.name}`);
        startedCount++;
      } catch (err) {
        console.error(`   ❌ Failed to start: ${streamData.name} - ${err.message}`);
        failedCount++;
      }
    } else {
      console.warn(`   ⚠️  Skipping (file not found): ${streamData.name}`);
      failedCount++;
    }
  });
  ...
  // ===== SUMMARY =====
  console.log('\n' + '='.repeat(60));
  console.log('✅ STREAM SYNCHRONIZATION COMPLETE');
  console.log('='.repeat(60));
  console.log(`   Total streams in registry: ${streamsMetadata.length}`);
  console.log(`   Streams started: ${startedCount}`);
  console.log(`   Failed to start: ${failedCount}`);
  console.log(`   New streams added: ${addedCount}`);
  console.log(`   Orphaned entries removed: ${orphanedCount}`);
  console.log(`   Invalid entries cleaned: ${removedCount}`);
  console.log('='.repeat(60) + '\n');
}
