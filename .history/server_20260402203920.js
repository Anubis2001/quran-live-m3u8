  ...
  try {
    ...
    const killProcess = () => {
      return new Promise((resolve, reject) => {
        ...
        // Check every 500ms if process is dead, force kill after 3 seconds
        ...
        // Force kill timeout after 3 seconds
        const forceKillTimeout = setTimeout(() => {
          clearInterval(checkInterval);
          
          if (isProcessRunning(pid)) {
            console.warn(`Process ${pid} didn't terminate gracefully, force killing`);
            try {
              ...
              if (process.platform === 'win32') {
                ...
                proc.kill('SIGKILL');
                console.log(`Force killed process ${pid} using SIGKILL`);
              }
              ...
              setTimeout(() => {
                if (!isProcessRunning(pid)) {
                  ...
                  console.error(`Failed to kill process ${pid} even with force kill`);
        ...
        setTimeout(() => {
          ...
          if (isProcessRunning(pid)) {
            console.error(`Process ${pid} still running after 5 seconds, giving up`);
            reject(new Error(`Process ${pid} did not terminate`));
          } else {
            resolve(true);
          }
        }, 5000);
