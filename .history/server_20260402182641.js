...
// PUBLIC: Serve HLS files without auth
app.use("/streams", express.static(path.join(__dirname, "streams")));

// ADMIN AUTH only for dashboard & API
app.use(
  ["/", "/api", "/api/{*rest}"],
  basicAuth({ users: { admin: "@!JKF3eWd12" }, challenge: true })
);
...
app.get("/api/streams", (req, res) => {
  const list = Object.keys(runningStreams).map(name => {
    ...
    const status = isAlive ? "running" : entry.failed ? "failed" : "stopped";
    return { name, status };
  });
  res.json(list);
});
