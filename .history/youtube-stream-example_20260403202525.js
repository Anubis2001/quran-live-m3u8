/**
 * Universal Audio HLS Streaming - Usage Examples
 * 
 * This demonstrates how to stream audio from any platform supported by yt-dlp/youtube-dl
...
/**
 * Start a universal audio stream (supports multiple platforms)
 * @param {string} name - Stream name (will be used in URL)
 * @param {string} mediaUrl - Media URL from any supported platform (YouTube, Instagram, SoundCloud, etc.)
...
async function startYoutubeStream(name, mediaUrl, cookiesPath) {
  try {
    ...
    const requestBody = { url: mediaUrl };
    if (cookiesPath) {
...
async function main() {
  console.log('========================================');
  console.log('Universal Audio HLS Streaming Examples');
  console.log('========================================\n');
  console.log('Supported platforms: YouTube, Instagram, SoundCloud, TikTok, Twitter/X, Facebook, and 1000+ more\n');
  
  // Example 1: Start streaming from a YouTube video (without authentication)
  console.log('Example 1: Start Quran Recitation Stream (YouTube)');
  console.log('--------------------------------------------------------');
  try {
  ...
  // Example 2: Start streaming with cookies authentication
  console.log('\n\nExample 2: Start Stream with Cookies Authentication');
  console.log('---------------------------------------------------');
  console.log('For age-restricted or private videos:');
  try {
    await startYoutubeStream(
      'private-recitation',
      'https://www.youtube.com/watch?v=PRIVATE_VIDEO_ID',
      '/root/cookies.txt'  // Path to your Netscape-format cookies file
    );
  } catch (e) {
    console.log('Note: Replace with actual YouTube URL and cookies.txt path');
    console.log('Cookies file should be in Netscape HTTP Cookie format');
  }
  
  // Example 3: Start streaming from Instagram
  console.log('\n\nExample 3: Start Instagram Audio Stream');
  console.log('----------------------------------------');
  try {
    await startYoutubeStream(
      'instagram-audio',
      'https://www.instagram.com/reel/EXAMPLE_REEL_ID'
    );
  } catch (e) {
    console.log('Note: Replace with actual Instagram URL');
  }
  
  // Example 4: Start streaming from SoundCloud
  console.log('\n\nExample 4: Start SoundCloud Stream');
  console.log('-----------------------------------');
  try {
    await startYoutubeStream(
      'soundcloud-track',
    ...
    console.log('Note: Replace with actual SoundCloud URL');
  }
  
  // Example 5: List all streams
  console.log('\n\nExample 5: List All Active Streams');
  console.log('-----------------------------------');
  await listStreams();
  ...
  // Example 7: Multiple listeners can access the same stream
