...
// PUBLIC: serve streams without auth
...
// return stream list with status
...
app.post("/api/upload", upload.single("audio"), (req, res) => {
  ...
  // build a full absolute URL based on incoming request
  const baseUrl = `${req.protocol}://${req.get("host")}`;
  const streamUrl = `${baseUrl}/streams/${name}/stream.m3u8`;
